<?php
    ...
    if(!empty($_GET['id'])){
        ...
        $id = checkInput($_GET['id']);//verifi ls variables opur evite les hackers
    ...
    $statement = $db->prepare('SELECT items.id, items.name, items.description, items.price, items.image, categories.name AS category 
                                                    ...
                                                    WHERE items.id=?');
    $statement->execute(array($id));//pas besoin d'une boucle puisqu'il y a qu'un ligne
    $item = $statement->fetch();//recup une ligne tout est stocker dans la variable
    Database::disconnect();
